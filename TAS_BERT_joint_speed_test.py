...
def main():
	...
	speed_fp = 'results/laptop/three_joint/BIO/train_eval_time.txt'
	...
	epoch=0
	model_name = "TAS-BERT"
	n_aspect = os.path.basename(args.data_dir)[7:]
	train_time = 0
	eval_time = 0
	for _ in range(int(args.num_train_epochs)):	
	# for _ in trange(int(args.num_train_epochs), desc="Epoch"):
		epoch+=1
		print("[{}] Training Epoch: {}".format(datetime.datetime.now(), epoch))
		model.train()
		tr_loss = 0
		tr_ner_loss = 0
		...
		for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
			...
			if(step == 20):
				end_time = time.time()
				train_time = (end_time - start_time) * n_batches / 20
				break
		...
		if args.eval_test and epoch % args.test_freq == 0:
			...
			with open(os.path.join(args.output_dir, "test_ep_"+str(epoch)+".txt"),"w") as f_test:
				...
				for input_ids, input_mask, segment_ids, label_ids, ner_label_ids, ner_mask in test_dataloader:
					if idx == 20:
						end_time = time.time()
						eval_time = (end_time - start_time) * n_batches / 20
						speed_file.write(",".join([model_name, n_aspect, str(train_time), str(eval_time)]) + "\n")
						speed_file.flush()
						break
